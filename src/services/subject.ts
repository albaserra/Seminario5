...
const getListaUsuarios=async(id:string)=>{
    const responseItem=await SubjectModel.findById({_id:id}).populate('users');
    return responseItem;
};

const getListaAsignaturas=async(id:string)=>{
    const responseItem=await SubjectModel.find({users:id}).populate('users');
    return responseItem;
};

export { insertSubject, getSubject, getSubjects, updateSubject, deleteSubject, matriculateSubject, getListaUsuarios, getListaAsignaturas};
